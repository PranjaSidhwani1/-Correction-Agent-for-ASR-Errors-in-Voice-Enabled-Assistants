...
class Agent(object):
    ...
    def asr_corrector(self, environment):
        ...
        best_cost = current_cost = 1#environment.compute_cost(current_state)
        visited=[]
        is_insertion=[0,0]
        ...
        while T > T_min:
            ...
            if len(neighbor_words) > len(current_words):
                if neighbor_words[0] != current_words[0]:
                    self.first_insertion = True
                if neighbor_words[-1] != current_words[-1]:
    ...
    def generate_informed_neighbors(self, state, environment, visited,is_insertion,iteration):
        ...
        if neighbors:
            ...
            neighbors_with_costs = [
                (s[0], s[1], random.uniform(0, 1),s[2]) for s in valid_neighbors
            ]
...
a=Agent(phoneme_table,vocabulary)
text="SHE FOLBED AMONG TE FLICKEWING SHADOWS A GRACEFUL AND HARMONIOUS IMAGE"
environment = Environment(text, None, phoneme_table)
a.asr_corrector(environment)
